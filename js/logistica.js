...
function imprimirHojaCarga(){
  ...
  const bloques=peds.map((p,i)=>{
    ...
    const filasProd=(p.items||[]).map((it,j)=>{
      ...
      return '<tr>'
        +'<td style="padding:1px 3px;border:1px solid #ccc;text-align:center;font-size:9px;color:#555;white-space:nowrap">'+codProd+'</td>'
        +'<td style="padding:1px 3px;border:1px solid #ccc;font-weight:500">'+it.nom+'</td>'
        +'<td style="padding:1px 3px;border:1px solid #ccc;text-align:center">'+it.cant+'</td>'
        +'<td style="padding:1px 3px;border:1px solid #ccc;text-align:center;color:#555">'+unitDisplay+'</td>'
        +'<td style="padding:1px 3px;border:1px solid #ccc;text-align:center;letter-spacing:1px">'+(esPeso?'_____':'—')+'</td>'
        +'</tr>';
    }).join('');
    const codCliente=c.codigo||p.cliente_id||'';

    return '<div style="margin-bottom:0;page-break-inside:avoid">'
      +'<div style="background:#1a7a52;color:#fff;padding:3px 6px;font-weight:700;font-size:10px;border-radius:3px 3px 0 0;line-height:1.3">'
        +(i+1)+'. <span style="font-size:12px;font-weight:900;background:rgba(255,255,255,0.2);padding:0 4px;border-radius:2px;letter-spacing:1px">'+codCliente+'</span> '+p.cliente
        +'<br><span style="font-size:9px;font-weight:400">'+(c.direccion||'')+' '+(p.localidad||'')+'</span>'
      +'</div>'
      +'<table style="width:100%;border-collapse:collapse">'
        +'<thead><tr style="background:#e8f5e9">'
          +'<th style="padding:1px 3px;border:1px solid #ccc;text-align:center;width:10%;font-size:9px">Cód.</th>'
          +'<th style="padding:1px 3px;border:1px solid #ccc;text-align:left;font-size:9px">Producto</th>'
          +'<th style="padding:1px 3px;border:1px solid #ccc;text-align:center;width:12%;font-size:9px">Cant.</th>'
          +'<th style="padding:1px 3px;border:1px solid #ccc;text-align:center;width:10%;font-size:9px">Unid.</th>'
          +'<th style="padding:1px 3px;border:1px solid #ccc;text-align:center;width:20%;font-size:9px">Peso real</th>'
        +'</tr></thead>'
        +'<tbody>'+filasProd+'</tbody>'
      +'</table>'
      +'<div style="text-align:right;padding:1px 4px;border:1px solid #ccc;border-top:none;font-size:9px;background:#fafafa">'
  ...
  w.document.write('<!DOCTYPE html><html><head><title>Hoja de Carga #'+cg.id+'</title>'
    +'<style>'
    +'body{font-family:Arial,sans-serif;padding:10px;color:#000;font-size:10px;margin:0}'
    +'@page{size:A4 portrait;margin:8mm}'
    +'@media print{.no-print{display:none}}'
    +'.hc-grid{display:grid;grid-template-columns:1fr 1fr;gap:6px 8px}'
    +'</style></head><body>'
    +'<div style="display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2px solid #1a7a52;padding-bottom:5px;margin-bottom:8px">'
      +'<div>'
        +'<div style="font-size:13px;font-weight:700;color:#1a7a52">🌸 DISTRIBUIDORA LILA — Hoja de Carga</div>'
        +'<div style="font-size:10px;margin-top:2px"><b>Repartidor:</b> '+vendedor+' &nbsp;&nbsp; <b>Fecha:</b> '+fecha+' &nbsp;&nbsp; <b>Carga #:</b> '+cg.id+'</div>'
      +'</div>'
